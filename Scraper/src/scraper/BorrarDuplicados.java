				...
				for (File file1 : listOfFiles) {
					...
					if (!tablaArchivos.containsKey(nomb_arch)) {
						...
						if (fecha_string1.compareTo(fecha_string2) < 0) {
							File archivoBorrar = new File (medioPrensa.getPath() + File.separator + fecha_string1 + File.separator + nomb_arch);
							archivoBorrar.delete();
							tablaArchivos.put(nomb_arch, fecha_string2);
						} else if (fecha_string1.compareTo(fecha_string2) > 0) {
							File archivoBorrar = new File (medioPrensa.getPath() + File.separator + fecha_string2 + File.separator + nomb_arch);
							archivoBorrar.delete();
						} else {
